...
class Parser:
    def __init__(self, config_name='allegro', config={}, quantity=10,
        ...
        self.filename = os.path.join(
            get_base_folder_path(), f'parsed_data\\{username} {datetime.now().strftime("%b %d %Y %H-%M-%S")}'
                                    f'.{extension}')
    ...
    def verify_config(self):
        required_fields = {'UrlResolveRules': ['PageLinkTemplate', ],
    ...
    def parse_single_item(self, url: str, item_id: int):
        ...
        if 'SimpleFields' in self.config.keys():
            for field_name, xpath in self.config['SimpleFields'].items():
                try:
                    xpath = xpath
        ...
        if item_dict:
            ...
            with open(self.filename, 'a+') as outfile:
                json.dump(item_dict, outfile, ensure_ascii=False)
